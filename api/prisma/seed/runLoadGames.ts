...
/**
 * Main function to run the loadGames script standalone.
 *
 * This function truncates the `games` table, fetches the first existing operator,
 * and then calls the original `loadGames` function with the operator ID.
 *
 * @remarks It assumes that the main seed script has been run first to seed an operator.
 */
async function main() {
